...
def pad_dir_seq(dir_seq, num_layers):
    if len(dir_seq) > num_layers:
        raise ValueError(f"Length of dir_seq '{dir_seq}' exceeds number of layers, '{num_layers}'.")
...
class SAGESwitchModel(nn.Module):
    def __init__(self,
        ...
        if num_layers == 1:
            self.sage_layers.append(OneDirSAGEConv(hidden_in, hidden_out, 'mean', feat_drop, norm=last_norm))
        else:
            # input projection (no residual)
            self.sage_layers.append(OneDirSAGEConv(
                hidden_in, num_hidden, aggregator_type='mean', feat_drop=feat_drop,
                activation=create_activation(activation), norm=norm))
            # hidden layers

            for l in range(1, num_layers - 1):
                self.sage_layers.append(OneDirSAGEConv(
                    in_feats=num_hidden, out_feats=num_hidden, feat_drop=feat_drop, aggregator_type='mean',
                    activation=create_activation(activation), norm=norm))

            # output projection
            self.sage_layers.append(OneDirSAGEConv(
                num_hidden, hidden_out, aggregator_type='mean', feat_drop=feat_drop,
