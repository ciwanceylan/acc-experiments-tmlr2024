...
from .acc import ACCGraphSAGE, get_emb_dims
from .dir_graphsage import SAGEMultiSwitchModel
from .pcapass import PCAPassGraphSAGE
...
class MLPDecoder(nn.Module):
    ...
    def __init__(self, in_dim: int, num_hidden: int, out_dim: int, *args, **kwargs):
        super().__init__()
...
def setup_module(directed, m_type, enc_dec, in_dim, max_dim, num_hidden, out_dim, num_layers, dropout, activation, norm,
                 bias: bool = False, min_add_dim: int = 2) -> nn.Module:
    if m_type == "mlp":
        # * just for decoder
        mod = MLPDecoder(in_dim=in_dim, num_hidden=num_hidden, out_dim=out_dim)
        # mod = nn.Sequential(
        #     nn.Linear(in_dim, num_hidden),
        #     nn.PReLU(),
        #     nn.Dropout(0.2),
    ...
    match m_type:
        ...
        case 'dir_graphsage':
            mod = SAGEMultiSwitchModel(
                dir_seqs=('OU', 'IU') if is_encoding else ('U',),
                in_dim=in_dim,
        ...
        case _:
            raise ValueError(f"Unknown model '{m_type}'")
