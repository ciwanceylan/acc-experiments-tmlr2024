...
class DirGAT(nn.Module):
    def __init__(self,
        ...
        if num_layers == 1:
            self.gat_layers.append(DirGATConv(
                in_dim, out_dim, nhead_out,
                feat_drop=feat_drop, attn_drop=attn_drop, negative_slope=negative_slope, residual=last_residual,
                norm=last_norm, concat_out=concat_out))
        else:
            # input projection (no residual)
            self.gat_layers.append(DirGATConv(
                in_dim, num_hidden, nhead,
                feat_drop=feat_drop, attn_drop=attn_drop, negative_slope=negative_slope, residual=residual,
                activation=create_activation(activation), norm=norm, concat_out=concat_out))
            # hidden layers
            for l in range(1, num_layers - 1):
                ...
                self.gat_layers.append(DirGATConv(
                    ...
                    negative_slope=negative_slope, residual=residual, activation=create_activation(activation),
...
class DirGATConv(nn.Module):
    ...
    def forward_one_dir(self, graph, feat):
    ...
    def forward(self, graph, feat):
        fwd_rst = self.forward_one_dir(graph, feat)
        bwd_rst = self.forward_one_dir(graph.reverse(), feat)
        rst = self._alpha * fwd_rst + (1 - self._alpha) * bwd_rst
        if self.activation:
            rst = self.activation(rst)
        return rst