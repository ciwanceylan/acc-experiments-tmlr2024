...
class DirGCN(nn.Module):
    ...
    def forward(self, graph, x):
        ...
        x = self.alpha * self.fwd_convs[-1](graph, x) + (1 - self.alpha) * self.bwd_convs[-1](rev_graph, x)

        return x


class CCA_SSG(nn.Module):
    def __init__(self, in_dim, hid_dim, out_dim, n_layers, directed: bool, use_mlp = False):
        super().__init__()
        if not use_mlp:
            if directed:
                self.backbone = DirGCN(in_dim, hid_dim, out_dim, n_layers)
            else:
