...
import accnebtools.ssgnns.acc_pcapass_gnn.trainutils as trainutils
...
def get_trainer(graph: dgraphs.SimpleGraph, args, use_cpu: bool):
    ...
    params = trainutils.Parameters(
        ...
        decoder='dir_graphsage',
    )
    model_trainer = trainutils.GraphMAE2Trainer(graph=graph, params=params, device=device)
...
def main():
    name = "accgnn"
    parser = parsing.make_parser(f"{METHOD_DIR}/config.yml", 'accgnn')
    parser.description = f"{name}: Self-supervised GNN version of ACC and PCAPass."
