...
import accnebtools.ssgnns.spgcl.trainutils as spgcl
...
def get_trainer(graph: dgraphs.SimpleGraph, args, use_cpu: bool):
    ...
    params = spgcl.Parameters(
        spgcl_params=spgcl.SPGCLParams(
            hidden=args.dimensions,
            encoder_layer_num=args.num_layers,
        ),
        subg_num_hops=args.subg_num_hops,
        lr=args.lr,
        wd=args.wd,
    ...
    model_trainer = spgcl.SPGCLTrainer(graph=graph, params=params, device=device)
...
def main():
    name = "spgcl"
    parser = parsing.make_parser(f"{METHOD_DIR}/config.yml", 'spgcl')
