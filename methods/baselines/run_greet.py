...
import accnebtools.ssgnns.greet.trainutils as greet
...
def get_trainer(graph: dgraphs.SimpleGraph, args, use_cpu: bool):
    ...
    params = greet.Parameters(
        aug_params=greet.AugmentParams(maskfeat_rate_1=args.dfr1, maskfeat_rate_2=args.dfr2,
                                       dropedge_rate_1=args.der1, dropedge_rate_2=args.der2,
                                       ),
        disc_params=greet.DiscParams(margin_hom=args.margin_hom, margin_het=args.margin_het),
        lr_gcl=args.lr,
        lr_disc=args.lr,
        cl_rounds=args.cl_rounds,
        wd=args.wd,
        num_neighbours=args.num_neighbours,
        dropout=args.dropout,
        emb_dim=args.dimensions,
        proj_dim=args.dimensions,
        nlayers_enc=args.num_layers,
        alpha=args.alpha,
        add_degree=args.add_degree,
        add_lcc=args.add_degree,
        standardize=args.standardize
    )
    model_trainer = greet.GREETTrainer(graph=graph, params=params, device=device)
...
def main():
    name = "greet"
    parser = parsing.make_parser(f"{METHOD_DIR}/config.yml", 'greet')
